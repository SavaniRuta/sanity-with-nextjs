    ...
    >
      {blocks.map((block) => {
        ...
        switch (block._type) {
          ...
          default: {
            // The switch covers every block type in the schema, so TypeScript
            // narrows `block` to `never` here. This is still reachable at
            // runtime when Studio has a block type this component doesn't
            // render yet, so read the fields off a widened type.
            const { _key, _type } = block as { _key: string; _type: string };
